defmodule MicroblogWeb.KeywordControllerTest do
  ...
  describe "index" do
    test "lists all keywords", %{conn: conn} do
      conn = get conn, keyword_path(conn, :index)
  ...
  describe "new keyword" do
    test "renders form", %{conn: conn} do
      conn = get conn, keyword_path(conn, :new)
  ...
  describe "create keyword" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, keyword_path(conn, :create), keyword: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == keyword_path(conn, :show, id)

      conn = get conn, keyword_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Show Keyword"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, keyword_path(conn, :create), keyword: @invalid_attrs
      assert html_response(conn, 200) =~ "New Keyword"
    end
  end
  ...
  describe "edit keyword" do
    ...
    test "renders form for editing chosen keyword", %{conn: conn, keyword: keyword} do
      conn = get conn, keyword_path(conn, :edit, keyword)
  ...
  describe "update keyword" do
    ...
    test "redirects when data is valid", %{conn: conn, keyword: keyword} do
      conn = put conn, keyword_path(conn, :update, keyword), keyword: @update_attrs
      assert redirected_to(conn) == keyword_path(conn, :show, keyword)

      conn = get conn, keyword_path(conn, :show, keyword)
      assert html_response(conn, 200) =~ "some updated keyword"
    end

    test "renders errors when data is invalid", %{conn: conn, keyword: keyword} do
      conn = put conn, keyword_path(conn, :update, keyword), keyword: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Keyword"
    end
  end
  ...
  describe "delete keyword" do
    ...
    test "deletes chosen keyword", %{conn: conn, keyword: keyword} do
      conn = delete conn, keyword_path(conn, :delete, keyword)
      assert redirected_to(conn) == keyword_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, keyword_path(conn, :show, keyword)
      end
    end
  end
